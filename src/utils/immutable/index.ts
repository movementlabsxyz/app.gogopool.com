...
export function createSerializationStream(data: any, options: SerializationStreamOptions = {}) {
  const indentation = options.pretty ? 2 : 0;
  const replacer = options.bigChunks ? serializeData : serializeDataAsync;

  const stream = new JSONStreamStringify(data, replacer, indentation, undefined);
  return stream;
}
