...
import { Collection, Record, isCollection } from 'immutable'
...
export function serializeArray(arr: any[], serializeChild: typeof serializeData): any[] {
  return arr.map((value, index) => {
    return serializeChild(index, value)
  })
}

export function serializePlainObject(obj: object, serializeChild: typeof serializeData): object {
  const objData: any = {}
...
export function serializeRecord(
  ...
  const recordName =
    Record.getDescriptiveName(record) || (record as any).displayName || (record as any)._name
...
export function serializeDataAsync(_key: string | number, value: any): any {
  let result = value
